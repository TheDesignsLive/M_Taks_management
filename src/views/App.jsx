...
export default function App() {
  ...
  const beamsRef = useRef(null); // ── UI State ──
  ...
  const closeAlert = () => setAlert((a) => ({ ...a, show: false })); // ── Login ──

  const [loginType, setLoginType] = useState("user");
  const [loginEmail, setLoginEmail] = useState("");

  const [loginPassword, setLoginPassword] = useState(""); // ── Signup ──
  ...
  const [profilePic, setProfilePic] = useState(null); // ── Errors ──

  const [phoneError, setPhoneError] = useState("");
  const [passError, setPassError] = useState("");
  const [confirmError, setConfirmError] = useState(""); // ── Forgot Password ──

  const [contact, setContact] = useState("");
  const [otp, setOtp] = useState("");
  const [generatedOtp, setGeneratedOtp] = useState(null);
  const [otpSent, setOtpSent] = useState(false); // ── Reset Password ──
...
window.__beamsClient = beamsClient;
  ...
  async function handleSignup(e) {
    ...
    if (signupPassword !== confirmPassword) {
      setConfirmError("Passwords mismatch.");
      hasError = true;
    }
    if (hasError) return;
  ...
  async function sendOtp() {
    if (!contact) {
      showAlert("Error", "Please enter email or phone", false);
      return;
    }
    ...
    try {
      ...
      if (data.status === "not_found") {
        ...
        showAlert(
          "Not Found",
          "Email or Phone not found in our records.",
          false,
        );
        setContact("");
        return;
  ...
  function verifyOtp() {
    if (Number(otp) === generatedOtp) {
      ...
      setOtp("");
      showAlert("Invalid", "Incorrect OTP. Try again.", false);
    }
  }
  ...
  async function resetPassword() {
    ...
    if (!passwordRegex.test(newPass)) {
      setResetPassError("Weak password.");
      hasError = true;
    }
    if (newPass !== confPass) {
      setResetConfError("Passwords mismatch.");
      hasError = true;
    }
    if (!newPass || !confPass) {

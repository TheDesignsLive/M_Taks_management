...
async function forwardMemberImageToDesktop(localFilename) {
  try {
    ...
    if (!contentType.includes("application/json")) {
      ...
      console.error(
        "[view_member] Desktop returned non-JSON:",
    ...
    console.error(
      "[view_member] forwardMemberImageToDesktop error:",
      err.message,
    );
    safeDeleteFile(localFilename);
    return null;
...
async function getAdminId(session) {
  ...
  const [rows] = await con.query("SELECT admin_id FROM users WHERE id=?", [
    session.userId,
  ]);
  ...
  upload.single("profile_pic")(req, res, async (uploadErr) => {
    ...
    try {
      ...
      if (canDirectInsert) {
        ...
        const [existing] = await con.query(
          'SELECT id FROM member_requests WHERE email=? AND status="PENDING"',
          [email],
        );
        if (existing.length) {
          if (profilePic) safeDeleteFile(profilePic);
        ...
        const [requesterRows] = await con.query(
          "SELECT name FROM users WHERE id=?",
          [senderId],
        );
        const requesterName = requesterRows.length
          ? requesterRows[0].name
          : "A member";
        ...
        return res.json({
          success: true,
          message: "Member request submitted. Awaiting admin approval.",
          isRequest: true,
        });
      }
    } catch (err) {
      ...
      if (tempPic) {
        ...
        if (user.profile_pic) {
          try {
            ...
            console.error("[view_member] delete old image error:", e.message);
          }
        }
...
router.post("/remove-pic/:id", requireAuth, async (req, res) => {
  try {
    ...
    const [rows] = await con.query(
      "SELECT profile_pic FROM users WHERE id=? AND admin_id=?",
      [id, adminId],
    );
    if (!rows.length)
      return res
    ...
    if (rows[0].profile_pic) {
      ...
      try {
        ...
        console.error("[view_member] Desktop delete-image failed:", e.message);
      }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to remove picture." });
  }
});
...
router.get("/suspend/:id", requireAuth, async (req, res) => {
  try {
    ...
    const [rows] = await con.query(
      "SELECT status FROM users WHERE id=? AND admin_id=?",
      [id, adminId],
    );
    if (!rows.length)
      return res
    ...
    if (canDirectDelete) {
      ...
      if (existing.length) {
        return res
          ...
          .json({
            success: false,
            message: "A pending delete request already exists for this member.",
          });
      }

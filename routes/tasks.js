...
router.post('/update-task-status', async (req, res) => {
    ...
    try {
        ...
        // ── When unchecking a repeat task, delete any future clone spawned ──
        if (status === 'OPEN') {
...
if (task.repeat_type && task.repeat_type !== 'none' && task.due_date) {
                    ...
                    console.log('[UNCHECK DELETE tasks.js] nextDateStr:', nextDateStr, '| title:', task.title);
                    ...
                    await db.execute(
                        `DELETE FROM tasks
                         ...
                         LIMIT 1`,
                        [task.admin_id, task.title, task.assigned_to, task.repeat_type, nextDateStr, id]
                    );
                }
            }
        }

        // ── Repeat logic: spawn next task on complete ──
        if (status === 'COMPLETED') {
            const [rows] = await db.execute('SELECT * FROM tasks WHERE id=?', [id]);
            if (rows.length > 0) {
                const task = rows[0];
                ...
                const taskDateStr = task.due_date instanceof Date
                    ? task.due_date.toISOString().split('T')[0]
                    : task.due_date ? String(task.due_date).split('T')[0] : null;
                ...
                if (repeatType && repeatType !== 'none') {
                    ...
                    // Guard: skip if exact next date clone already exists
                    const [existingClones] = await db.execute(
                        `SELECT id FROM tasks
                         WHERE admin_id=? AND title=? AND assigned_to=?
                           AND repeat_type=? AND status='OPEN' AND id != ?
                           AND DATE(due_date) = DATE(?)
                         LIMIT 1`,
                        [task.admin_id, task.title, task.assigned_to, repeatType, id, nextDateStr]
                    );
                    ...
                    if (existingClones.length === 0) {
                        await db.execute(
                            ...
                            [task.admin_id, task.title, task.description, task.priority,
                             nextDateStr, task.assigned_by, task.assigned_to, task.who_assigned, repeatType]
                        );
                    }

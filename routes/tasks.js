...
// ==============================
// GET TEAMS (for assign dropdown)
// ==============================
router.get('/get-teams', async (req, res) => {
  try {
    if (!req.session.role) return res.json({ success: false, message: 'Unauthorized' });

    const adminId = req.session.role === 'admin' ? req.session.adminId : req.session.adminId;
  ...
  try {
      ...
      for (const user of users) {
        await db.execute(
          ...
          [admin_id, title || 'No Title', description || null,
           (priority || 'MEDIUM').toUpperCase(), finalDate, user.id, assigned_by, who_assigned]
        );
      }
    ...
    if (assignedTo === 'all') {
      ...
      await db.execute(
        ...
        [admin_id, title || 'No Title', description || null,
         (priority || 'MEDIUM').toUpperCase(), finalDate, assigned_by, who_assigned]
      );

      return res.json({ success: true, message: 'Task added successfully' });
    }

    // ── SELF (assignedTo === 'self') ──
    if (assignedTo === 'self') {
    ...
    await db.execute(
      `INSERT INTO tasks
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'OPEN')`,
      [admin_id, title || 'No Title', description || null,
       (priority || 'MEDIUM').toUpperCase(), finalDate, finalAssignedTo,
       assigned_by, who_assigned, sectionValue]
    );
    ...
    await db.execute(
      `UPDATE tasks 
       SET title = ?, description = ?, priority = ?, due_date = ?, assigned_to = ?, 
           section = IF(status='COMPLETED', section, ?)
       WHERE id = ?`,

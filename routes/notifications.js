...
router.get("/", async (req, res) => {
  ...
  try {
    const canManageAnnounce =
      role === "admin" || control_type === "ADMIN" || control_type === "OWNER";
    ...
    if (canManageMembers) {
      const [mReqs] = await con.query(
        `SELECT mr.*, r.role_name, u.name AS requested_by_name FROM member_requests mr JOIN roles r ON r.id=mr.role_id JOIN users u ON u.id=mr.requested_by WHERE mr.admin_id=? AND mr.status='PENDING' AND mr.request_type='ADD' ORDER BY mr.created_at DESC`,
        [adminId],
      );
      const [dReqs] = await con.query(
        `SELECT mr.*, r.role_name, u.name AS requested_by_name FROM member_requests mr JOIN roles r ON r.id=mr.role_id JOIN users u ON u.id=mr.requested_by WHERE mr.admin_id=? AND mr.status='PENDING' AND mr.request_type='DELETE' ORDER BY mr.created_at DESC`,
        [adminId],
      );
  ...
  async (req, res) => {
    try {
      ...
      if (req.file) {
        try {
          ...
          const uploadRes = await fetch(
            `${DESKTOP_BASE_URL}/upload-attachment-base64`,
            {
              method: "POST",
              headers: {
          ...
          if (uploadData.success) {
            ...
            console.log(
              "[Mobile] ✅ Attachment uploaded to desktop:",
              desktopFilename,
            );
          } else {
          ...
          console.error(
            "[Mobile] ❌ Attachment upload failed:",
            uploadErr.message,
          );
        }
        try {
      ...
      const [rows] = await con.query(
        ...
        [result.insertId, req.session.adminId],
      );
      ...
      try {
        ...
        await beamsClient.publishToInterests(
          [companyAdminInterest],
          pushPayload,
        );
        console.log(
          "[Beams] Company admins notified via:",
          companyAdminInterest,
        );
        console.log(
          "[Mobile] 🔔 Push sent for announcement:",
          ann.id,
          "→ interests:",
          interests,
        );
      ...
      fetch(`${DESKTOP_BASE_URL}/api/notify-announcement-add`, {
        ...
        console.error(
          "[Mobile] notifyDesktop announcement_add failed:",
          err.message,
        ),
      );
...
router.get("/delete-announcement/:id", async (req, res) => {
  try {
    ...
    fetch("https://tms.thedesigns.live/api/notify-announcement-delete", {
      ...
      console.error("[Mobile] notifyDesktop delete failed:", err.message),
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const [check] = await con.query(
        "SELECT id FROM announcements WHERE id=? AND admin_id=?",
        [announcementId, req.session.adminId],
      );
      if (!check.length)
        return res.status(403).json({ success: false, message: "Not found" });
      ...
      if (req.file) {
        try {
          ...
          const uploadRes = await fetch(
            `${DESKTOP_BASE_URL}/upload-attachment`,
            {
              method: "POST",
              headers: {
                "x-mobile-secret": MOBILE_SECRET,
          ...
          if (!uploadRes.ok) {
            console.error(
              "[Mobile] ❌ Desktop upload HTTP error:",
              uploadRes.status,
            ...
            if (uploadData.success) {
              desktopFilename = uploadData.filename;
              console.log(
                "[Mobile] ✅ Edit attachment uploaded to desktop:",
                desktopFilename,
              );
            } else {
              console.error("[Mobile] ❌ Desktop rejected:", uploadData);
          ...
          console.error(
            "[Mobile] ❌ Edit attachment upload failed:",
            uploadErr.message,
          );
        }
        try {
      ...
      fetch(`${DESKTOP_BASE_URL}/api/notify-announcement-edit`, {
        ...
        console.error("[Mobile] notifyDesktop edit failed:", err.message),
      );
